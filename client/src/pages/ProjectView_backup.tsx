            ...
            <div className="flex-1 flex overflow-hidden">
              ...
              <div className="w-1/2 bg-surface border-r border-gray-200 flex flex-col">
                <div className="p-6 border-b border-gray-200">
                  ...
                  <p className="text-sm text-gray-600 mt-1">Track progress through each stage of your colorblock workflow</p>
                </div>

                <div className="flex-1 overflow-y-auto p-6">
                  {/* Always start by showing workflow stage cards, drill down when test is selected */}
                  {!selectedTestId ? (
                    // Show workflow stage cards when no test is selected
                    <div className="grid grid-cols-1 gap-4">
                      {(() => {
                        // Calculate workflow stage counts
                        const newTestsCount = tests.filter(test => 
                          test.statements.some((s: any) => s.status === 'draft')
                        ).length;
                        
                        const pendingReviewCount = tests.filter(test => 
                          test.statements.some((s: any) => s.status === 'under_review' || s.status === 'needs_revision')
                        ).length;
                        
                        const readyToDeployCount = tests.filter(test => 
                          test.statements.some((s: any) => s.deploymentStatus === 'ready')
                        ).length;
                        
                        const completedCount = tests.filter(test => 
                          test.statements.some((s: any) => s.deploymentStatus === 'completed')
                        ).length;
                        ...
                        return (
                          <>
                            ...
                            <div
                              ...
                              onClick={() => window.location.href = '/dashboard/new-tests'}
                              data-testid="card-stage-new-tests"
                            >
                              <div className="flex items-center justify-between">
                                <div className="flex items-center">
                                  ...
                                  <div>
                                    <h3 className="text-lg font-semibold text-gray-900">New Tests</h3>
                                    <p className="text-sm text-gray-600">Draft statements waiting to be submitted</p>
                                  </div>
                                </div>
                                <div className="text-right">
                                  <div className="text-2xl font-bold text-gray-900">{newTestsCount}</div>
                                  <div className="text-sm text-gray-500">Active{newTestsCount !== 1 ? '' : ''}</div>
                                </div>
                              </div>
                            </div>
                            ...
                            <div
                              className="bg-white border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow cursor-pointer"
                              onClick={() => window.location.href = '/dashboard/pending-review'}
                              data-testid="card-stage-pending-review"
                            >
                              <div className="flex items-center justify-between">
                                <div className="flex items-center">
                                  ...
                                  <div>
                                    ...
                                    <p className="text-sm text-gray-600">Statements awaiting growth strategist approval</p>
                                  </div>
                                </div>
                                <div className="text-right">
                            ...
                            <div
                              className="bg-white border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow cursor-pointer"
                              onClick={() => window.location.href = '/dashboard/ready-to-deploy'}
                              data-testid="card-stage-ready-deploy"
                            >
                              <div className="flex items-center justify-between">
                                <div className="flex items-center">
                                  <div className="w-12 h-12 bg-green-100 rounded-lg flex items-center justify-center mr-4">
                                    <i className="fas fa-rocket text-xl text-green-600"></i>
                                  </div>
                                  <div>
                                    <h3 className="text-lg font-semibold text-gray-900">Ready to Deploy</h3>
                                    <p className="text-sm text-gray-600">Approved colorblocks ready for Facebook ads</p>
                                  </div>
                                </div>
                                <div className="text-right">
                                  <div className="text-2xl font-bold text-green-600">{readyToDeployCount}</div>
                            ...
                            <div
                              ...
                              data-testid="card-stage-completed"
                            >
                              <div className="flex items-center justify-between">
                                <div className="flex items-center">
                                  <div className="w-12 h-12 bg-blue-100 rounded-lg flex items-center justify-center mr-4">
                                    <i className="fas fa-check-circle text-xl text-blue-600"></i>
                                  </div>
                                  <div>
                                    <h3 className="text-lg font-semibold text-gray-900">Completed</h3>
                                    <p className="text-sm text-gray-600">Deployed and completed test batches</p>
                                  </div>
                                </div>
                                <div className="text-right">
                    ...
                    // Show individual test cards when drilling down from stage cards
